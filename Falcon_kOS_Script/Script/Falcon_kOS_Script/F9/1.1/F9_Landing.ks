//Falcon kOS Script Falcon 9 Landing Ver 1.1

//------------------------------------------------------------------------

//Setup
runOncePath("0:/Falcon_kOS_Script/F9/1.1/F9_Library.ks").
global saveFileLocation to "0:/Falcon_kOS_Script/F9/1.1/F9_SaveFile.json".
...
//------------------------------------------------------------------------
    ...
    else if RunMode = 2
    {
        ...
        if FacingError(heading(-OP(FKS_config["lz-1"]["latlng"], -3000):heading, 180):vector) < 45
        {
    ...
    {
        if Error(impact["LatLng"], OP(FKS_config["lz-1"]["latlng"], -3000))[3] < 0
        {
            rcs on.
...
function Steer {
    ...
    {
        return lookdirup(vxcl(up:vector, -Error(impact["LatLng"], OP(FKS_config["lz-1"]["latlng"], -3000))[0]), -up:vector).
    }
    ...
    else if RunMode = 6
    {
        ...
        local TgP to OP(FKS_config["lz-1"]["latlng"], 400).

        return SteeringPid(line(10, 0.1, 150, 13, Error(IP, TgP)[1], 0.1, 13), 0.1, ship:velocity:surface, Error(IP, TgP)[0]).
    }
    else if RunMode = 7
    {
        if TrueAlt() - FKS_config["lz-1"]["alt"] < 100
        {
            return lookDirUp(ship:up:vector, facing:topvector).
        }
        return SteeringPid(line(150, -5, 1000, -10, Error(ship:geoPosition, FKS_config["lz-1"]["latlng"])[1], -10, -5), 3, ship:velocity:surface, Error(latlng((ship:geoposition:lat + impact["LatLng"]:lat)/2, (ship:geoposition:lng + impact["LatLng"]:lng)/2), FKS_config["lz-1"]["latlng"])[0]).
    }
    else if RunMode = 8
    {
        return lookDirUp(ship:up:vector, facing:topvector).
