//Falcon kOS Script Falcon Heavy Library Ver 1.0
...
function TrueAlt
{
    ...
    local Result to alt:radar - radarOffset.
...
function ImpactData
{
    ...
    if addons:tr:hasimpact
    {
        ...
        local tillImpact to addons:tr:timetillimpact.
...
function KillSurfVel
{
    parameter Aoa, Mag.

    local Result to -ship:velocity:surface * Mag.
    if vAng(Result, -ship:up:vector) > Aoa {set Result to ship:up:vector:normalized + tan(Aoa) * -ship:velocity:surface:normalized.}
