//Falcon kOS Script Falcon Heavy Landing Ver 1.0

//------------------------------------------------------------------------

//Setup
runOncePath("0:/Falcon_kOS_Script/FH/1.0/FH_Library.ks").
global saveFileLocation to "0:/Falcon_kOS_Script/FH/1.0/FH_SaveFile.json".
...
//------------------------------------------------------------------------
    ...
    if RunMode = 1
    {
           ...
           CheckList["MECO_StageSep"]
        {
            ...
            EngineMode("FH_Core_Engine", "Three").
            EngineMode("FH_Side_Engine_1", "Three").
            EngineMode("FH_Side_Engine_2", "Three").
    ...
    else if RunMode = 2
    {
        ...
        if FacingError(heading(-OP(LZ["latlng"], -3000):heading, 180):vector) < 45
        {
            ...
            Action("FH_Side_Engin_1", "ModuleEnginesFX", "Thrust Limiter", 100).
            Action("FH_Side_Engin_2", "ModuleEnginesFX", "Thrust Limiter", 100).
    ...
    else if RunMode = 5
    {
        ...
        if not CheckList["ToggleSoot"]
        {
            ...
            Action("FH_MainEngine", "ModuleTundraSoot", "Toggle Soot", true).
    ...
    else if RunMode = 6
    {
        ...
        if ship:altitude < 10000
        {
            ...
            {
                EngineMode("FH_Core_Engine", "Center").
                EngineMode("FH_Side_Engine", "Center").
    ...
    {
        //Center Engine Only
        if (Identifier[1] = "Core" and EngineList["FH_Core_Engine"]["CurtMode"] = "Three" and ThrottleControl(700, LZ["alt"], 0) < 1) or
           (Identifier[1] = "Side" and (Identifier[2] = "1" and EngineList["FH_Side_Engine_1"]["CurtMode"] = "Three" or Identifier[2] = "2" and EngineList["FH_Side_Engine_2"]["CurtMode"] = "Three") and ThrottleControl(700, LZ["alt"], 0) < 1)
        {
            EngineMode("FH_Core_Engine", "Center").
            EngineMode("FH_Side_Engine_1", "Center").
            EngineMode("FH_Side_Engine_2", "Center").
        }
...
function Thrott {
    ...
    {
        if (Identifier[1] = "Core" and EngineList["FH_Core_Engine"]["CurtMode"] = "Center") or
           (Identifier[1] = "Side" and Identifier[2] = "1" and EngineList["FH_Side_Engine_1"]["CurtMode"] = "Center") or
           (Identifier[1] = "Side" and Identifier[2] = "2" and EngineList["FH_Side_Engine_2"]["CurtMode"] = "Center")
        {
